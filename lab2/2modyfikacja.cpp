...
public:
    ...
    CNumber operator/(const CNumber &other) const;
    CNumber operator<<(const CNumber &other) const;
    CNumber operator<<(unsigned shift) const;
...
CNumber CNumber::operator<<(const CNumber &other) const {
    ...
    for (int i = 0; i < other.i_lenght; i++) {
        shift += other.pi_number[i] *pot ;
        pot *=10;
    }
    shift = shift % i_lenght;

    for (int i = 0; i < i_lenght; i++) {
        result.pi_number[i] = pi_number[(i + i_lenght - shift) % i_lenght];
    }
...
CNumber CNumber::operator<<(unsigned shift) const {
    CNumber result(*this);
    CNumber two;
    two=2;
...
int main() {
    ...
    cout<< (c_num_6 << c_num_3).sToStr() << endl;
    cout<< (c_num_6 << 3).sToStr() << endl;
    cout<< c_num_6 << endl;
